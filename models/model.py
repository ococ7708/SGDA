...
class SFE(nn.Module):   # Shared Feature Extractor
    def __init__(self, num_electrodes=62, num_freq_bands=5, dropout=0.3, final_dim=512):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                in_channels=1,
                out_channels=16,         # 输出特征图个数设置为16，因为单层卷积需要更多特征图来捕捉复杂模式
                kernel_size=(5, 3),      
                #   - Height=5: 一次看 1个电极的所有频带 (正好5个频带)
                #   - Width=3:  一次看完所有时间步 (T=3)

                stride=(1, 1),           # 步长设为 1，精细扫描
                # stride=(2, 1),   # 步长也可以设置2：将 310 的高度特征图压缩为 155，去除冗余，保留关键模式

                padding=(2, 0),          # H方向padding=2保持高度，W方向padding=0变成1
                bias=False
            ),
            nn.BatchNorm2d(16),    # batchNnormalization 加速训练收敛，稳定训练过程  参数和输出特征图数量一致
            nn.ELU()
        )
        ...
        # 卷积后 H 保持 310 (因为 pad=2, k=5)，W 变为 1 (因为 k=3, T=3, pad=0)   输出形状: [B, 16, 310, 1]
        self.flat_dim = 16 * (num_electrodes * num_freq_bands) * 1    #卷积步长为stride=(1, 1)的情况

        # conv_out_height = (num_electrodes * num_freq_bands + 1) // 2 #卷积步长为stride=(2, 1)的情况
    ...
    def forward(self, x):
        # Input x: [B, T, C, Fre] = [B, 3, 62, 5]
        B, T, C, Fre = x.shape
        
        # -----------------------------------------------------------
        # 关键变换逻辑：
        # 1. permute: 把 T 移到最后 -> [B, C, Fre, T]
        # 2. view: 合并 C 和 Fre -> [B, 1, C*Fre, T]
        # 这样 reshape 后，内存排列是 E1_F1, E1_F2... E1_F5, E2_F1...
        # 也就是"同一个电极的频带"是相邻的，符合卷积核(5,3)的设计初衷
        # -----------------------------------------------------------
        x = x.permute(0, 2, 3, 1).contiguous().view(B, 1, C * Fre, T)
        
        x = self.conv(x)  # -> [B, 16, 310, 1]
...
# ==========================================
# 2. 域特定分支 (DSFE) Domain-Specific Feature Extractor
# ==========================================
...
class SGDA_model(nn.Module):
    ...
    def forward(self, x_src_list, x_tgt, return_sfe_features=False):
            ...
            x_tgt: Tensor
            return_sfe_features: 是否返回 SFE 切空间特征（用于几何正则化）
            """
            z_src_all = []
            src_feat_list = []

...
def run_experiment(args):
    ...
    args.LM = "clip"  #支持的文本编码器：- clip - bert - sbert - roberta_go
    ...
    for session_idx in range(n_sessions):
        ...
        for target_sub in range(n_subjects):
            ...
            for epoch in range(args.epochs):
                ...
                # ===== 在 target 上评估 =====
                y_true, y_pred = get_preds(    #也可以用get_preds_avg，不需要算source_centroids_tensor，效果差不多，更快
                    model,
                    dl_tgt_eval,
                    prototypes_list,
    ...
    out_path = os.path.join(
        args.output_dir,
        f"csubs_seed_bd{bd_dim}_bs{args.batch_size}.csv"
    )

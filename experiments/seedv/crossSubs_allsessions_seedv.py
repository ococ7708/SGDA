import os
import sys
import torch
import torch.optim as optim
import numpy as np
from sklearn.metrics import f1_score
from torch.utils.data import TensorDataset, DataLoader

# ===== path =====
current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(os.path.dirname(current_dir))
sys.path.append(project_root)

# ===== data & utils =====
from data_utils.load_seedv import read_seedv_feature, segment
from data_utils.text_to_vector import label_to_vector
from data_utils.constants.path_mapper import path_mapper

from utils.args import get_args_parser

from config.setting import preset_setting, set_setting_by_args
from utils.mix_utils import (
    setup_seed,
    flatten_trials,
    zscore_subject_seedv,
    get_preds
)

from utils.log_utils import save_csubs_results_csv
from utils.loss import align_loss, mmd_linear, discrepancy_loss

# ===== model =====
from models.model_seedv import SGDA_model


def load_data(args, device):
    """Load SEED-V features and text embeddings"""
    if args.setting is not None:
        setting = preset_setting[args.setting](args)
    else:
        setting = set_setting_by_args(args)

    setting.dataset_path = path_mapper['seedv_de_lds']
    setting.dataset = 'seedv'
    setting.experiment_mode = 'subject-independent'
    setting.onehot = False
    setting.sample_length = 3
    setting.sessions = [1, 2, 3]
    setting.stride = 1

    # feature extraction + temporal segmentation
    data, label = read_seedv_feature(setting.dataset_path, setting.sessions)
    data, label = segment(data, label, setting.sample_length, setting.stride)

    text_dim, allClassLabelvector = label_to_vector(
        dataset=setting.dataset,
        LM=args.LM,
        device=device
    )

    data, label = flatten_trials(data, label)

    return data, label, allClassLabelvector, 62, 5, text_dim


def run_experiment(args):
    """Cross-subject + cross-session (merge all sessions per subject)"""

    setup_seed(args.seed)

    args.device = torch.device("cuda:1")
    args.class_ids = [0, 1, 2,3,4]
    args.LM = "clip"

    args.epochs = 200
    args.batch_size = 128
    args.lr = 1e-3

    args.experiment_name = 'results_seedv'
    args.output_dir = os.path.join(project_root, f"results/{args.experiment_name}")
    os.makedirs(args.output_dir, exist_ok=True)

    print(f"Output dir: {args.output_dir}")

    bd_dim = 128

    # ===== data =====
    (all_data, all_label, allClassLabelvector,
     num_electrodes, num_freq_bands, text_dim) = load_data(args, args.device)

    all_data = zscore_subject_seedv(all_data)

    # ---- merge sessions (key setting) ----
    # treat each subject as a single domain by concatenating all sessions
    n_sessions = len(all_data)
    n_subjects = len(all_data[0])

    merged_data, merged_label = [], []

    for sub in range(n_subjects):
        sub_data = []
        sub_label = []

        for ses in range(n_sessions):
            sub_data.append(all_data[ses][sub])
            sub_label.append(all_label[ses][sub])

        merged_data.append(np.concatenate(sub_data, axis=0))
        merged_label.append(np.concatenate(sub_label, axis=0))

    print(f"Merged sessions. #Subjects: {n_subjects}, Samples per subject: {merged_data[0].shape[0]}")

    # text prototypes (fixed semantic anchors)
    sorted_indices = sorted(allClassLabelvector.keys())
    vectors_in_order = [allClassLabelvector[i] for i in sorted_indices]

    text_prototypes = torch.tensor(
        np.array(vectors_in_order, dtype=np.float32)
    ).to(args.device)

    prototypes_list = text_prototypes

    results_acc, results_macrof1, results_microf1 = [], [], []

    # ===== LOSO over subjects =====
    for target_sub in range(n_subjects):
        setup_seed(args.seed)

        print(f"\nTarget Subject {target_sub} (all sessions)")

        source_ids = [s for s in range(n_subjects) if s != target_sub]

        # ----- build source loaders -----
        source_loaders = []
        for sid in source_ids:
            X = torch.tensor(np.array(merged_data[sid]), dtype=torch.float32).to(args.device)
            y = torch.tensor(np.array(merged_label[sid]), dtype=torch.long).to(args.device)

            ds = TensorDataset(X, y)
            dl = DataLoader(ds, batch_size=args.batch_size, shuffle=True, drop_last=True)
            source_loaders.append(dl)

        # ----- target loaders -----
        X_tgt = torch.tensor(np.array(merged_data[target_sub]), dtype=torch.float32).to(args.device)
        y_tgt = torch.tensor(np.array(merged_label[target_sub]), dtype=torch.long).to(args.device)

        ds_tgt = TensorDataset(X_tgt, y_tgt)
        dl_tgt_train = DataLoader(ds_tgt, batch_size=args.batch_size, shuffle=True, drop_last=True)
        dl_tgt_eval = DataLoader(ds_tgt, batch_size=args.batch_size, shuffle=False)

        n_sources = len(source_loaders)

        model = SGDA_model(
            n_sources,
            num_electrodes,
            eeg_dim=text_dim,
            bottleneck_dim=bd_dim,
            text_dim=text_dim,
            dropout=0.1
        ).to(args.device)

        optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-4)

        steps_per_epoch = min(len(dl) for dl in source_loaders)
        total_steps = args.epochs * steps_per_epoch

        tgt_iter = iter(dl_tgt_train)
        source_iters = [iter(dl) for dl in source_loaders]

        step = 0
        best_acc, best_macro, best_micro = 0, 0, 0

        # ===== training =====
        for epoch in range(args.epochs):
            model.train()
            epoch_loss = 0.0

            for _ in range(steps_per_epoch):
                step += 1

                # gradually increase adaptation strength
                gamma = 2.0 / (1.0 + np.exp(-10.0 * step / total_steps)) - 1.0
                alpha = float(gamma)
                beta = float(gamma) / 100

                x_src_list, y_src_list = [], []

                for i, src_iter in enumerate(source_iters):
                    try:
                        xb, yb = next(src_iter)
                    except StopIteration:
                        source_iters[i] = iter(source_loaders[i])
                        xb, yb = next(source_iters[i])

                    x_src_list.append(xb.to(args.device))
                    y_src_list.append(yb.to(args.device))

                try:
                    x_tgt, _ = next(tgt_iter)
                except StopIteration:
                    tgt_iter = iter(dl_tgt_train)
                    x_tgt, _ = next(tgt_iter)

                x_tgt = x_tgt.to(args.device)

                optimizer.zero_grad()
                z_src_all, z_tgt_all = model(x_src_list, x_tgt)

                total_cls, total_mmd = 0.0, 0.0
                for i in range(n_sources):
                    total_cls += align_loss(z_src_all[i], text_prototypes, y_src_list[i])
                    total_mmd += mmd_linear(z_src_all[i], z_tgt_all[i])

                total_cls /= n_sources
                total_mmd /= n_sources

                disc = discrepancy_loss(z_tgt_all, text_prototypes)

                loss = total_cls + alpha * total_mmd + beta * disc
                loss.backward()
                optimizer.step()

                epoch_loss += loss.item()

            if (epoch + 1) % 10 == 0:
                print(f"Epoch {epoch+1} | Loss={epoch_loss/steps_per_epoch:.4f}")

            # ----- compute source centroids -----
            model.eval()
            source_centroids = []

            with torch.no_grad():
                for i in range(n_sources):
                    feats = []
                    for xb_s, _ in source_loaders[i]:
                        f = model.sfe(xb_s.to(args.device))
                        f = model.branches[i](f)
                        feats.append(f)

                    centroid = torch.cat(feats, dim=0).mean(dim=0)
                    source_centroids.append(centroid)

            source_centroids = torch.stack(source_centroids, dim=0)

            # ----- evaluation -----
            y_true, y_pred = get_preds(
                model,
                dl_tgt_eval,
                prototypes_list,
                source_centroids=source_centroids,
                device=args.device,
                fusion_type='feature',
                tau=1.0
            )

            acc = (y_true == y_pred).mean()

            if acc > best_acc:
                best_acc = acc
                best_macro = f1_score(y_true, y_pred, average='macro')
                best_micro = f1_score(y_true, y_pred, average='micro')

                print(f"  -> best acc: {best_acc:.4f} (epoch {epoch})")

            if best_acc == 1.0:
                break

        print(f"Final acc (sub {target_sub}): {best_acc*100:.2f}%")

        results_acc.append(best_acc)
        results_macrof1.append(best_macro)
        results_microf1.append(best_micro)

    # ===== summary =====
    results_acc = np.array(results_acc)

    print(f"\nMean acc: {results_acc.mean()*100:.2f}%, std: {results_acc.std()*100:.2f}%")

    final_acc_dict = {'CrossSubsAllSession': results_acc}
    final_macro_dict = {'CrossSubsAllSession': results_macrof1}
    final_micro_dict = {'CrossSubsAllSession': results_microf1}

    out_path = os.path.join(
        args.output_dir,
        f"csubsess_seedv_bd{bd_dim}_bs{args.batch_size}.csv"
    )

    save_csubs_results_csv(final_acc_dict, final_macro_dict, final_micro_dict, out_path)
    print(f"Results saved to {out_path}")


if __name__ == '__main__':
    args = get_args_parser().parse_args()
    run_experiment(args)
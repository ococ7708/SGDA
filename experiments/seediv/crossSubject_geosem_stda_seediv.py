...
def _parse_session_indices(session_indices, n_sessions):
    if session_indices is None:
        return list(range(n_sessions))
    parsed = []
    for item in session_indices:
        for part in str(item).split(","):
            part = part.strip()
            if part:
                parsed.append(int(part) - 1)
    if not parsed:
        return list(range(n_sessions))
    if len(set(parsed)) != len(parsed):
        raise ValueError(f"Duplicate session id in --session_ids: {session_indices}")
    invalid = [idx + 1 for idx in parsed if idx < 0 or idx >= n_sessions]
    if invalid:
        raise ValueError(f"Invalid session ids {invalid}; dataset has {n_sessions} sessions")
    return parsed
...
def _make_optimizer(args, model):
    if not getattr(args, "use_param_groups", False):
        return optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    return optim.Adam(
        [
            {"params": model.encoder.parameters(), "lr": args.backbone_lr},
            {"params": model.adapters.parameters(), "lr": args.lr},
            {"params": model.prototype_head.parameters(), "lr": args.lr},
        ],
...
def _train_selection_warmup(model, source_loaders, target_loader, text_prototypes, class_weights, num_classes, args):
    ...
    for epoch in range(args.reliability_warmup_epochs):
        ...
        for _ in range(steps_per_epoch):
            ...
            if args.selection_warmup_proto_only:
...
def evaluate(model, loader, text_prototypes, source_centroids, device, proto_tau,
             fusion_tau, eval_classifier="text", centroid_blend=0.5,
             source_domain_centroids=None, source_reliability_weights=None,
    ...
    true_parts, pred_parts = predict_class_aware(
        ...
        reliability_fusion=reliability_fusion,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session_ids", type=str, nargs="+", default=None,
                        help="1-based SEED-IV session ids to run, e.g. --session_ids 1 or --session_ids 1 2 3")

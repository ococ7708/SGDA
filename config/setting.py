...
class Setting:
    def __init__(self, dataset, dataset_path, pass_band, extract_bands, time_window, overlap, sample_length, stride, seed,
                 ...
                 eog_clean=True,metrics=None, normalize=False, save_data=True, split_type="kfold", fold_num=5, fold_shuffle=True,
                 front=9, test_size=0.2, val_size = 0.2, sessions=None, pr=None, sr=None, bounds=None,onehot=False, label_used=None):
        # random seed
        self.seed = seed
        ...
        # Data at indices 0 and 1 represent the lower and higher thresholds of bandpass filtering下标 0 和 1 处的数据代表带通滤波的较低和较高阈值
...
def faced_sub_independent_train_val_test_setting(args):
    ...
    return Setting(dataset='faced_de', dataset_path='home/ch/FACED/', pass_band=[args.low_pass, args.high_pass],
                   extract_bands=None, time_window=args.time_window, overlap=args.overlap,
                   sample_length=args.sample_length, stride=args.stride, seed=args.seed, feature_type=args.feature_type,
...
def seed_sub_dependent_train_val_test_setting(args):
    ...
    #对于每个受试者，9条随机试次用作训练集，3条随机试次用作验证集，最后3条试验用作测试集。我们选取验证集中的最佳结果以及测试集中的测试结果
    return Setting(dataset=args.dataset, dataset_path=args.dataset_path, pass_band=[args.low_pass, args.high_pass],
                   extract_bands=None, time_window=args.time_window, overlap=args.overlap,
                   sample_length=args.sample_length, stride=args.stride, seed=args.seed, feature_type=args.feature_type,
                   only_seg=args.only_seg, experiment_mode="subject-dependent", normalize=args.normalize,
...
def seed_sub_independent_leave_one_out_setting(args):
    ...
    #使用留一法，将 1 名受试者 15 次试验的所有样本作为一个测试集，将其他 14 名受试者 15 次试验的所有样本作为一个训练集，循环 15 次以报告平均结果。
    return Setting(dataset=args.dataset, dataset_path=args.dataset_path, pass_band=[args.low_pass, args.high_pass],
                   extract_bands=None, time_window=args.time_window, overlap=args.overlap,
                   sample_length=args.sample_length, stride=args.stride, seed=args.seed, feature_type=args.feature_type,
                   only_seg=args.only_seg, experiment_mode="subject-independent", normalize=args.normalize,
...
def seed_sub_independent_train_val_test_setting(args):
    ...
    print("Using train-val-test SEED subject independent experiment mode,\n"
          ...
          "and test it in test set")
    #随机选取9个受试者的数据作为训练集，随机选取3个受试者的数据作为验证集，随机选取3个受试者的数据作为测试集。我们选择验证集中的最佳结果，并在测试集中进行测试。
    return Setting(dataset=args.dataset, dataset_path=args.dataset_path, pass_band=[args.low_pass, args.high_pass],
                   extract_bands=None, time_window=args.time_window, overlap=args.overlap,
                   sample_length=args.sample_length, stride=args.stride, seed=args.seed, feature_type=args.feature_type,
                   only_seg=args.only_seg, experiment_mode="subject-independent", normalize=args.normalize,

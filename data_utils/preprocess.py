...
def preprocess(data, baseline, sample_rate, pass_band, extract_bands, time_window, overlap, car=False, whiten=False,
    ...
    if not only_seg:  # seed_de_lds已经处理过了所以无需再经过带通滤波了
        if pass_band != [-1, -1]:
    ...
    data, feature_dim = segment_data(data, sample_length, stride)  # 数据和频段数
...
def bandpass_filter(data, frequency, pass_band):
    ...
    对脑电信号进行基带滤波操作。在信号处理领域，基带滤波通常是为了去除特定频率范围之外的信号成分，以突出或提取特定频率范围内的有用信息。
    input: EEG signal
    ...
    输出的维度和输入的数据维度一样，只有original_data->filter_data
    """
...
def feature_extraction(data, sample_rate, extract_bands, time_window, overlap, feature_type):
    ...
    filter_data 变成 band + band_feature
    """
    isLds = False
    if feature_type.endswith("_lds"):
        isLds = True
        feature_type = feature_type[:-4]
    fe = {#feature extraction的缩写
        'psd': psd_extraction,
        'de': de_extraction,
...
def lds(data):
    """
    Process data using a linear dynamic system approach.使用线性动态系统方法处理数据。
...
def segment_data(data, sample_length, stride):  # trail起底下就是np数组
    """
    feature:
    input: original band features of EEG signal provided by SEED dataset,SEED 数据集提供的 EEG 信号的原始波段特征
    output:
    ...
    output shape -> data: (session, subject, trail, sample, channel, seg_data_points)
                    label: (session, subject, trail)
    """
    if sample_length == 1:  # 样本长度为1 每个"分段"只包含原始数据中的1个时间点，相当于未分割。窗口长度=1相当于"无窗口"的退化情况
        print("每个subject的trial数：", len(data[0][0]))  # 每个subject的trial数量
        print("每个trial的时间点数（样本数）：", len(data[0][0][0]))  # 每个trail的时间点数(原来是通道数62）
        print("每个时间点的通道数数：", len(data[0][0][0][0]))  # 通道数62(原来是时间点235)
        print("频段数：", len(data[0][0][0][0][0]))
        return data, len(data[0][0][0][0][0])  # 返回原始数据和频段数
    else:  # >1的情况
        seg_data = []
        for ses_i, session in enumerate(data):
            ...
            for sub_i, subject in enumerate(data[ses_i]):
                ...
                for t_i, trail in enumerate(data[ses_i][sub_i]):
                    ...
                    elif len(trail.shape) == 2:  # 适用于原始数据
...
def label_process(data, label, bounds=None, onehot=False, label_used=None):
    ...
    if label_used is None:
        label_used = ['valence']  # 情绪效价包括消极，中性，积极三种
    ...
    for ses_i, ses_label in enumerate(label):
        ...
        for sub_i, sub_label in enumerate(ses_label):
            ...
            for trail_i, trail_label in enumerate(sub_label):
                ...
                if type(trail_label) is np.ndarray:  # 多个标签维度的处理情况
                    pro_label = []
                    ...
                    if len(pro_label) == len(used_id):
                        # 将多个二值标签组合成二进制字符串，并转为整数（如 [0,1] → "01" → 1）
                        trail_label = int("".join(str(i)for i in pro_label), 2)
                    else:
                        # discard the data and label 丢弃无效数据和标签
                        continue
                if onehot:
...
'''
整个 new_label 是一个嵌套的 Python list，
最外层：session 数
第二层：subject 数
第三层：trial 数
第四层（具体内容）：一个 NumPy 数组，shape = (sample, num_classes)，比如 (235, 3)
'''
